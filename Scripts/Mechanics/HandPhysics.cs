...
namespace Mechanics
{
    public class HandPhysics : MonoBehaviour
    {
        ...
        public Mechanics.HandCollider handColliderPrefab;
        [HideInInspector]
        public Mechanics.HandCollider handCollider;

        [Tooltip("Layers to consider when checking if an area is clear")]
        public LayerMask clearanceCheckMask;

        [HideInInspector]
        public Mechanics.Hand hand;
        ...
        private void Start()
        {
            hand = GetComponent<Mechanics.Hand>();
            //spawn hand collider and link it to us
            
            handCollider = ((GameObject)Instantiate(handColliderPrefab.gameObject)).GetComponent<Mechanics.HandCollider>();
            Vector3 localPosition = handCollider.transform.localPosition;
            Quaternion localRotation = handCollider.transform.localRotation;

            handCollider.transform.parent = PlayerController.instance.transform;
